     <style type="text/css">
                ...
                <ul class="list-group">
                    <?php
                         foreach ($sent as $message) {
                             ...
                             $path             =   new_url('message/sent/' . $offer_id ."/" . $received_userid); 
                             $val="<li class='list-group-item message' >
                                        <a href='". $path ."' class='row one-thread' data-offer_id='". $this->encrypt->encode($message['ride_offer_id']) ."' data-received_userid='".$this->encrypt->encode($message['received_userid'])."'  >
                                               ...
                                               <div class='col-lg-4 msg'>
                                                        <span class='badge ". (($message['readed_sender'] == 0) ? "unread" : "") ."' >".$message['number']."</span>
                                                        ". $origin . " → " . $destination ."
                                               </div>
                                               <div class='col-lg-5 msg'>
                                                ...
                                                if($message['readed_sender'] != 0){
                                                 ...
                                                 $val.=      "<span href='#block-user-modal-send' data-toggle='modal'  class='block-user-send' >
                         ...
                         if( count($sent) == 0 ){
                              echo " <div class='bs-example'>
                                         <div class='alert alert-dismissable alert-info'>
                                              ...
                                              <h4> ". lang("m.empty") ." <a href='". new_url('message/send') ."' style='margin:10px' title='". lang('refresh') ."' ><i class='glyphicon glyphicon-refresh'></i> </a> </h4>
                                         </div>
                                     </div>";
                         }  
                    ?>            
      ...
      <script src="<?php echo   base_url() . 'scripts/partial/messages/sent.js' ?>"></script>
