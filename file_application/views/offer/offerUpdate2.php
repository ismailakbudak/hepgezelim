                ...
                <div class="well ">
                    <form class="bs-example form-horizontal" onsubmit="return false">
                        <fieldset>
                               <legend> 
                                        <? 
                                              echo lang('ou.updateoffer'); 
                                              echo "<a href='". new_url('offer') ."'> <i title='". lang('od.offer') ."' class=' glyphicon glyphicon-briefcase one right'></i></a>";  
                                        ?>  
                               </legend>
                                           ...
                                           <div class='row'> 
                                                  <div class='input-group col-lg-12  form-padding2'>
                                                        <h4 class="header-title col-lg-8"> <?=lang('o2.blankseat')?> :</h4>
                                                         <div class="col-lg-4">
                                                              <span class='input-group-btn' >
                                                                     <input id="inputSeatCount"   value='<?= $offer['number_of_seats']?>' class='form-control input-sm inputNumber green'  style="float: left; width:80px; color: rgb(95, 95, 95);" data-pricemax='8' type='text' >
                                 ...
                                 <div class="panel panel-default">
                                      ...
                                      <div class="panel-body" id="">
                                            ...
                                            <div class="form-group"> 
                                               ...
                                               <div class="col-lg-12">
                                                 <textarea class="form-control"  title=' <?=lang('o2.movedescTitle')?> ' rows="3" id="inputExplainGoing" placeholder="<?=lang('o2.movedesc')?>" ><?= trim($offer['explain_departure']) ?></textarea>
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-group" id="TripReturnInformation">
                                                    ...
                                                    <div class="col-lg-12">
                                                      <textarea class="form-control" rows="3"   title=' <?=lang('o2.returndescTitle')?>' id="inputExplainReturn" placeholder="<?=lang('o2.returndesc')?>" ><?= trim($offer['explain_return'])?></textarea>
                                                    </div>
                                            </div>
                                              ...
                                              <?php 
                                                     $carsVal = ""; 
                                                     foreach ($cars as $car){
                                                            if(  strcmp( $offer['car_id'], $car['id'] ) ==  0 ) 
                                                                   $carsVal .= "<option selected value='{$car['id']}' > {$car['model']} </option>";
                                                            else
                                                          ...
                                                          <select class="form-control" id="luggages">
                                                            <?php
                                                                 ...
                                                                 foreach ($luggages as $luggage){
                                                                      ...
                                                                      $selected =  ( strcmp( $offer['luggage_id'], $luggage['id'] ) ==  0 ) ? "selected" : ""; 
                                                                      if( strcmp($this->lang->lang(), 'en') == 0  ) 
                                                                          echo "<option $selected value='{$luggage['id']}'>{$luggage['sizeen']}</option>";
                                                                      else
                                                                          echo "<option $selected value='{$luggage['id']}'>{$luggage['size']}</option>";
                                                                  }
                                                             ?>
                                                           </select>
                                                         </div>
                                                          ...
                                                          <select class="form-control" id="leave_times">
                                                            <?php
                                                                 ...
                                                                 foreach ($leave_times as $leave_time){
                                                                      $selected =  ( strcmp( $offer['leave_time_id'], $leave_time['id'] ) ==  0 ) ? "selected" : ""; 
                                                                      if( strcmp($this->lang->lang(), 'en') == 0  )  
                                                                          echo "<option $selected value='{$leave_time['id']}'>{$leave_time['timeen']}</option>";
                                                                      else  
                                                                          echo "<option $selected value='{$leave_time['id']}'>{$leave_time['time']}</option>";
                                                                  }    
                                                            ?>
                                                          </select>
                                                        </div>
                                 ...
                                 <div class="row">      
                                      <div class="form-group form-padding">
                                          <div class="col-lg-4" style="padding-bottom:5px">
                                             <button id="buttonBackForupdate2" type="button" class="form-control btn btn-default"> <?=lang('o2.back')?> </button> 
                                          </div>
                                          <div class="col-lg-8 ">
                                             <button id="inputUpdate" type="button" class="form-control btn btn-primary  apply-btn-loader"> <span class="img-loader hide"></span><?=lang('ou.update')?> </button> 
                                          </div>
                                      </div>
                                 </div>
   ...
   <div id="full-width" class="modal container fade" tabindex="-1" data-width="600" style="display: none;">
    ...
    <script type="text/javascript">           
          var isTwoway = '<?php echo $test["round_trip"]  ?>', pointsSession = '<?php echo $test["way_points"] ?>', startSession = '<?php echo $test["origin"] ?>', endSession = '<?php echo $test["destination"] ?>', offer_id   =  '<?php echo $test["offer_id_code"] ?>', findExpected = <?= json_encode($findExpected) ?>,  login = '1', range = 15 ;
    </script>
    <script type="text/javascript" >
            (function($){
                        $('body').modalmanager('loading');   
                        CapitiliazeFirst(  [ "#inputExplainReturn", "#inputExplainGoing" ]  );      
                        $( "#buttonBackForupdate2" ).on('click',function(){
                            window.location = base_url + "offer/updateToUpdate"; 
                        });
            })(jQuery);   
     </script>
